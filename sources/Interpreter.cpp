 ...
 */
 Interpreter::Interpreter(const string fileName) {
	...
	//interpreta a primeira linha que contêm a função objetivo e modo
...
void Interpreter::getObjectiveAndMode(string line) {
	...
	if(temp == "MAX") {
	    ...
	    throw(new Exception("Interpreter: Não encontrado modo (Maximizar ou Minimizar) da função objetivo!"));
	}
